...
package: $(TARGET)
	mkdir -p package/$(TARGET)-$(shell git describe --tags --always)
	cp $(TARGET) package/$(TARGET)-$(shell git describe --tags --always)
	cp -r kernels package/$(TARGET)-$(shell git describe --tags --always)
	ldd $(TARGET) | grep "=> /.[^/]" | awk '{print $$3}' | xargs -- cp -t package/$(TARGET)-$(shell git describe --tags --always) 
	bsdtar -czvf $(TARGET)-$(shell git describe --tags --always).zip -C package $(TARGET)-$(shell git describe --tags --always)
